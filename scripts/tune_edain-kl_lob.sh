...
declare -a power_lrs=(0.01 0.001 0.00001 0.0000001)
i=0
    ...
    for LR_shift in "${shift_lrs[@]}" ; do
        for LR_out in "${outlier_lrs[@]}" ; do
            for LR_pow in "${power_lrs[@]}" ; do
                CMD="CUDA_VISIBLE_DEVICES=2 python3 src/experiments/run_experiment.py --experiment-config src/experiments/configs/experiment-config-beta.yaml --device 0 --dataset lob --model gru-rnn --preprocessing-method standard-scaler --num-cross-validation-folds 1 --experiment-name edain-kl-lob-tuning-${i} --override='edain_bijector_fit:scale_lr:${LR_scale} edain_bijector_fit:shift_lr:${LR_shift} edain_bijector_fit:outlier_lr:${LR_out} edain_bijector_fit:power_lr:${LR_pow} fit:num_epochs:10' --edain-kl"
                i=$((i+1))
                eval "$CMD"
            done
        done 

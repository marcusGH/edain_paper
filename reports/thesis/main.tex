...
\marginpar{\scriptsize Double check $N$?}
dataset, contains $N=458\,913$ customers, entries of length $T=13$
...
In the second group, the mean $\kappa$ metric and mean macro-$F_1$ score is higher for the
...
folds where either \ac{BIN} or \ac{DAIN} outperform the proposed \ac{EDAIN} method.
...
No advantages, just limitations :(

\subsection{Limitations}%
\label{sub:Limitations}

% TODO TODO TODO: Write the limitations of the PREPMIX-CAPS next...

Very computationally inefficient

Not adaptive, so cannot learn a local-aware transformation like \ac{EDAIN},
\ac{BIN} and \ac{DAIN}, so not suitable for highly multimodal datasets

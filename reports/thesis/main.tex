...
In this thesis, we want to construct automatic preprocessing layers and assess how well
they can handle predictor variables of with different distribution
characteristics. As such, it will be very useful to have full control over how
the covariates are distributed, ideally through only needing to specify an
arbitrary unnormalized \ac{pdf} function for each variable.  Additionally, we
want the generated data to have a covariance structure that resembles that of a
multivariate time-series. Lastly, as the time-series will be used in supervised
learning, in this case a classification problem, we also want to generate a
response $y \in \{0,1\}$ that is based on the covariates. During research, I
was unable to find any algorithms that met all of these criteria. As such,  I
created my own data generation procedure that meets all of these criteria,
while being very computationally efficient.

We start by providing a general overview of the generation procedure, then go more in-depth into
each part of it later.
The main input to the data generation procedure is the multivariate time-series
length $T \in \mathbb{N}$ and the number of predictor variables at each
timestep, denoted $d \in \mathbb{N}$. For each predictor variable $j=1,2,\dots,d$,
...
multivariate time-series covariate and a corresponding response in three steps,
each step providing one of the functional requirements we highlighted earlier. 
Note that this procedure is repeated $N$ times if we want to, say, generate a
dataset of $N$ samples.
An overview of the these three steps of the data generation algorithm is shown in
\cref{eq:synth_algo1,eq:synth_algo2,eq:synth_algo3} below. Each row corresponds to the different
...
\begin{figure}
    \begin{center}
        \includegraphics[width=\textwidth]{figures/synthetic_data_example_transformations.pdf}
    \end{center}
    \caption{TODO}
...
Write down the PDFs, and refer to their plots. Also refer to appendix where all hyperparameters
specified.

Refer to example transformation plots and explain CDF inversion method

Then refer to table of results, and do a bit of discussion, highlighting how performance improved
when apply preprocessing, especially with CDF inversion where data is normally distributed

Talk about how this is motivating examples for the research question in general
...
\begin{figure}
\begin{center}
    \includegraphics[width=\textwidth]{figures/amex_data_eda_plot.pdf}
\end{center}
\caption{
...
\begin{figure}
    \begin{center}
        \includegraphics[width=\textwidth]{figures/lob_data_eda_plot.pdf}
    \end{center}
    \caption{
...
\caption{
    ...
    data points
    $\mathcal{X}=\left\{x^{(i)}_{j,t}\right\}_{i=1,2,\dots,N,t=1,2,\dots,T}$,
...
\marginpar{\scriptsize I will fix these empty pages at the end with some \texttt{$\backslash$vspace*\{-Xcm\} }}

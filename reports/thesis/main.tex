...
\marginpar{\scriptsize Comment: In this copy, I have set the line-spacing to \texttt{$\backslash$onehalfspacing} (1.5 times the regular line-spacing) as I believe it makes the paragraphs nicer to read. Let me know if I should revert this change.}
...
\marginpar{\scriptsize TODO? Describes how this ``cumulative moving average estimate'' is computed?}
...
\marginpar{\scriptsize Mention somewhere that is biased estimate of kurotisis and skewness being used here.}
...
real-world dataset \citep{nawi, brits}. As such, if one discovers novel preprocessing methods that
...
\marginpar{\scriptsize Dropout is not explained anywhere. Reader might get confused.}
...
\marginpar{\scriptsize This page will not be a single line when I add the synthetic data experiments}
\ac{LOB} dataset.
...
We now present some insights related to the performance of the proposed \ac{EDAIN} method,
...
First of all, the we note that the \ac{BIN}, \ac{DAIN} and local-aware \ac{EDAIN} methods are
...
This can be explained by the nature of the \ac{LOB} datasets: It contains limit orders from several
...
condition the normalization on what data generation mechanism the sample
...
than the preprocessing methods they treat all the samples the same, hence observation (i).
...
In the Amex datasets, the samples are not that different. They all come from a
similar data generation mechanism, so can be thought of as just random samples
...
Observations when it comes to performance:
* It outperforms standard scaling and min-max scaling on the \ac{LOB} dataset
* It is beaten by standard scaling on Amex dataset, even though it should theoretically be able
  to generalise standard scaling with its shift and scale layers
...
Additionally, these task are not required to fit into the deep learning
framework because the \ac{EDAIN-KL} bijector is trained completely separate of
...
machine learning models not based on neural network.
...
\marginpar{\scriptsize Does this PCA example make sense?}
...
invert these summaries to get an expression for the inverse shift and scale operation. However,
...
so it cannot be used for highly multimodal datasets such as the \ac{LOB} dataset.

\marginpar{\scriptsize Should the paragraph on ``why this method perform poorly
  on datasets'' be in the ``Limitations'' subsection? Or make separate subsection
  for that point?
}
...
datasets that the methods have been evaluated on in this thesis. Although the
...
Performance-wise, the \ac{PREPMIX-CAPS} routine has the second-best performance on the Amex dataset
out of the methods compared, outperforming standard scaling slightly while underperforming the
proposed global-aware \ac{EDAIN} method. However, the method comes with several limitations.

\subsection{Limitations}%
\label{sub:Limitations}

One of \ac{PREPMIX-CAPS}'s biggest limitations is its computational inefficiency. As discussed in
\cref{sub:prep_determine}, to determine what preprocessing method to use for each cluster of predictor variables, 
$(m-1)k+1$ experiments need to be ran. Even though this search can be parallelised across multiple \acp{GPU}, it still takes
a significant amount of time, especially compared to the other preprocessing methods such as Standard scaling and
the overhead of \ac{EDAIN}, \ac{BIN} or \ac{DAIN}. This problem becomes even worse when it comes to tuning the hyperparameter $k$,
that is, the number of clusters. For each candidate value for $k$, we need to run all the $(m-1)k+1$ experiments again.
In addition to the slow fit procedure, the preprocessing method also leads to slow convergence when fitting the neural network.
As we saw \cref{fig:amex_performance}, with the \ac{PREPMIX-CAPS} procedure, the neural network took a bit more than twice as many
epochs to converge compared to with standard scaling or \ac{EDAIN}.
A third limitation is that the method cannot perform local-aware transformations like the \ac{EDAIN}, \ac{BIN} and \ac{DAIN} layers,
as the procedure is not an adaptive one. It might therefore not be suitable for highly multimodal datasets unless one of the $m$
...
% TODO: can do brief summary here, then move detailed explanation to the appendix, such
%       as what parameters used to sample the scale and shift ....
